                ...
                Self::Watcher { diagnostics } => {
                        ...
                        inputs.recv_any(&cancellation).await?
                    {
                        ...
                        if cancellation_requested {
                            return Err(ConduitError::cancelled(WATCHER_CANCEL_REASON));
                        }
                    }

                    Err(ConduitError::execution(
...
fn assert_expected_output(
    summary: &conduit_engine::WorkflowRunSummary,
...
fn packet_payload_string(packet: PortPacket) -> conduit_core::Result<String> {
    let bytes: Vec<u8> = packet
        ...
        .ok_or_else(|| ConduitError::execution("watcher workload expected byte payload"))?;
