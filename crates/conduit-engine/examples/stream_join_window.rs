            ...
            match self {
                ...
                Self::Sink { received } => {
                    ...
                    let rows: Vec<String> = packets
                        ...
                        .collect::<conduit_core::Result<Vec<String>>>()?;
                    received
                        .lock()
...
impl JoinState {
    fn accept(&mut self, port_id: &PortId, payload: &str) -> conduit_core::Result<Option<String>> {
        match port_id.as_str() {
            "events" => self.accept_event(payload),
            "profiles" => self.accept_profile(payload),
            other => Err(ConduitError::execution(format!(
                "join-window received packet on unexpected input `{other}`"
            ))),
        }
    }

    fn accept_event(&mut self, payload: &str) -> conduit_core::Result<Option<String>> {
    ...
    fn accept_profile(&mut self, payload: &str) -> conduit_core::Result<Option<String>> {
...
fn parse_event(payload: &str) -> conduit_core::Result<EventRecord> {
...
fn parse_profile(payload: &str) -> conduit_core::Result<ProfileRecord> {
...
fn payload_fields(payload: &str) -> conduit_core::Result<[&str; 4]> {
...
fn packet_payload_string(packet: PortPacket) -> conduit_core::Result<String> {
    let bytes: Vec<u8> = packet
        .into_payload()
        .as_bytes()
        .map(|bytes| bytes.to_vec())
        .ok_or_else(|| ConduitError::execution("stream join workload expected byte payload"))?;
    String::from_utf8(bytes)
        .map_err(|source| ConduitError::execution(format!("payload was not UTF-8: {source}")))
}
...
fn metadata_jsonl_from_sink(
    ...
    String::from_utf8(bytes)
        .map_err(|source| ConduitError::metadata(format!("metadata JSONL was not UTF-8: {source}")))
}
