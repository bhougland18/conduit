...
fn mock_llm_decision(prompt: &str) -> conduit_core::Result<String> {
...
fn mock_tool_execute(call: &str) -> conduit_core::Result<String> {
...
fn mock_llm_finalize(context: &str) -> conduit_core::Result<String> {
...
fn packet_payload_string(pkt: PortPacket) -> conduit_core::Result<String> {
    let bytes = pkt
        ...
        .map(|b| b.to_vec())
        .ok_or_else(|| {
            ConduitError::execution("ai-call-orchestration workload expected byte payload")
        })?;
