...
Conduit is an experimental Flow-Based Programming engine in Rust. It validates
...
If you want a mental model, think of Conduit like a shipping hub:
...
Conduit is based on J. Paul Morrison's original Flow-Based Programming model.
...
- The same design gives Conduit room to run native nodes, WASM nodes, and
...
Conduit is trying to be the kind of system you can
...
WASM is useful in Conduit because it gives you a narrow, portable execution
boundary without asking the rest of the system to trust a whole host runtime.

The value here is not just “WASM is safer.” It is that WASM gives Conduit a
...
- A WASM node is easier to ship and inspect than a native plugin with deep host
  reach.
- The host owns the channels, so the graph still looks like Conduit even when a
...
Tokio is a strong general-purpose async ecosystem, but Conduit does not need a
...
should execute Conduit's model, not define a second one.

That is why `asupersync` sits underneath `conduit-runtime` rather than being
the public model itself.

- Conduit owns the graph, ports, contracts, metadata, and capability story.
...
- The public API does not inherit Tokio’s larger vocabulary when Conduit only
  needs a smaller one for workflow execution.

There is nothing wrong with Tokio in general. The point is simply that Conduit
...
#table(
  ...
  [`error`], [`terminal error object or null`], [`mirrors stable Conduit error shape`],
...
If those three disagree, Conduit treats it like a wiring mismatch and stops
...
#table(
  ...
  [`conduit-core`], [`Runtime interfaces and metadata shape.`], [`Defines the Conduit-owned runtime vocabulary.`],
  [`conduit-runtime`], [`The `asupersync` adapter boundary.`], [`Hides substrate details behind Conduit types.`],
...
Conduit works best when the workflow document, runtime behavior, and capability
