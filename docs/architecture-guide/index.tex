...
\urlstyle{same}
\hypersetup{
  pdftitle={Conduit Architecture Guide},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{Conduit Architecture Guide}
\author{}
\date{}
\begin{document}
\maketitle
...
\chapter{Conduit Architecture Guide}\label{conduit-architecture-guide}
...
\section{What Conduit Is}\label{what-conduit-is}

Conduit is an experimental Flow-Based Programming engine in Rust. It
...
At a high level, Conduit separates:
...
\NormalTok{    WF[Workflow Documents\textbackslash{}nJSON/TOML/YAML] {-}{-}\textgreater{} WFF[conduit{-}workflow{-}format\textbackslash{}nRaw parse + version checks]}
\NormalTok{    WFF {-}{-}\textgreater{} W[conduit{-}workflow\textbackslash{}nGraph structure validation]}

\NormalTok{    W {-}{-}\textgreater{} C[conduit{-}contract\textbackslash{}nContracts + schema compatibility]}
\NormalTok{    W {-}{-}\textgreater{} I[conduit{-}introspection\textbackslash{}nPure projections]}
\NormalTok{    C {-}{-}\textgreater{} I}
\NormalTok{    CAP[conduit{-}core capability model] {-}{-}\textgreater{} C}

\NormalTok{    C {-}{-}\textgreater{} E[conduit{-}engine\textbackslash{}nWorkflow orchestration]}
\NormalTok{    CORE[conduit{-}core\textbackslash{}nPorts, metadata, errors, traits] {-}{-}\textgreater{} E}
\NormalTok{    R[conduit{-}runtime\textbackslash{}nasupersync adapter] {-}{-}\textgreater{} E}
\NormalTok{    WASM[conduit{-}wasm\textbackslash{}nWasmtime batch adapter] {-}{-}\textgreater{} E}

\NormalTok{    E {-}{-}\textgreater{} CLI[conduit{-}cli\textbackslash{}nvalidate/inspect/explain/run]}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{conduit-types} & Validated identifier primitives
(\texttt{WorkflowId}, \texttt{NodeId}, \texttt{PortId}) & all crates \\
\texttt{conduit-workflow-format} & External format decode + versioning
(\texttt{conduit\_version}) & CLI + loaders \\
\texttt{conduit-workflow} & Static topology model + structural
validation & contract/introspection/engine \\
\texttt{conduit-core} & Runtime-facing traits, ports, metadata, errors,
capabilities & runtime/engine/cli/wasm \\
\texttt{conduit-contract} & Node contract model + validation against
workflow/capabilities & engine/introspection/cli \\
\texttt{conduit-introspection} & Read-only workflow+contract+capability
projections & CLI inspect/explain \\
\texttt{conduit-runtime} & \texttt{asupersync} runtime wrapper + node
observer boundary & engine \\
\texttt{conduit-engine} & Workflow orchestration, registry scheduling,
summary/policy & CLI \\
\texttt{conduit-wasm} & Wasmtime Component Model batch execution adapter
& engine/CLI \\
\texttt{conduit-cli} & User and automation entrypoint & humans/CI \\
\texttt{conduit-test-kit} & Builders and helpers for tests/examples &
tests/docs \\
\end{longtable}
...
\NormalTok{    participant CLI as conduit{-}cli}
\NormalTok{    participant Format as workflow{-}format}
\NormalTok{    participant Graph as workflow/contract}
\NormalTok{    participant Engine as conduit{-}engine}
\NormalTok{    participant Runtime as conduit{-}runtime}
...
Conduit emits two machine-facing outputs:
...
\item
  \texttt{conduit\ run\ -\/-json} terminal summary.
\end{itemize}
...
\ExtensionTok{cargo}\NormalTok{ run }\AttributeTok{{-}p}\NormalTok{ conduit{-}cli }\AttributeTok{{-}{-}}\NormalTok{ validate examples/native{-}linear{-}etl.workflow.json}

\CommentTok{\# Inspect topology/contracts/capabilities}
\ExtensionTok{cargo}\NormalTok{ run }\AttributeTok{{-}p}\NormalTok{ conduit{-}cli }\AttributeTok{{-}{-}}\NormalTok{ inspect examples/native{-}linear{-}etl.workflow.json}

\CommentTok{\# Run and emit machine{-}facing summary}
\ExtensionTok{cargo}\NormalTok{ run }\AttributeTok{{-}p}\NormalTok{ conduit{-}cli }\AttributeTok{{-}{-}}\NormalTok{ run }\AttributeTok{{-}{-}json} \DataTypeTok{\textbackslash{}}
\NormalTok{  examples/native{-}linear{-}etl.workflow.json }\DataTypeTok{\textbackslash{}}
\NormalTok{  /tmp/conduit{-}native{-}linear.metadata.jsonl}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{\{}
  \DataTypeTok{"conduit\_version"}\FunctionTok{:} \StringTok{"1"}\FunctionTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{use} \PreprocessorTok{conduit\_contract::}\OperatorTok{\{}
...
\OperatorTok{\};}
\KeywordTok{use} \PreprocessorTok{conduit\_core::}\OperatorTok{\{}
...
\tightlist
\item
  \texttt{conduit-workflow} validates structural graph honesty only.
\item
  \texttt{conduit-contract} validates contract/capability alignment plus
  schema compatibility across connected ports.
\item
  \texttt{conduit-core} owns runtime interfaces and metadata shape.
\item
  \texttt{conduit-runtime} is an adapter over \texttt{asupersync}, not
  the public model.
\item
  \texttt{conduit-wasm} isolates Wasmtime and WIT/component ABI details
  from core.
\end{itemize}
