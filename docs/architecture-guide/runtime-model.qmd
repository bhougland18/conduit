...
The short version is that Conduit tries to make execution feel ordinary:
...
Conduit runtime behavior is coordinated by three layers:
...
Before any node logic runs, Conduit enforces static safety:
...
ideas together; Conduit keeps them separate so each one stays simpler.
...
Conduit uses stable error taxonomies (`CDT-*`) and structured error payloads so
...
- Cancellation is represented as a shared Conduit-owned signal, so a node can
...
- The runtime wrapper intentionally stays narrow. Most of the interesting
  behavior belongs to Conduit itself, not to the async substrate.
