...
> Guidelines for AI coding agents working in this Rust codebase. This local copy is being adapted for JJ-based bead workflows used in our projects.

---

## Rule 0 - Human Instructions Win

If the human gives a direct instruction, follow it even when it conflicts with a lower-priority guideline in this file.

---

## Rule 1 - No File Deletion Without Permission

Do not delete files or folders without explicit permission from the human. That includes files you created earlier in the session.
...
## Version Control: Use JJ Per Bead

This adaptation assumes the repo uses `jj` for version control.

- Keep one bead per JJ change when practical.
- Put the bead ID in the JJ change description and any final commit/message summary.
- Do not bundle unrelated beads into one change.
- If a bead is rolled back, restore the JJ change to match the rollback boundary.
- Prefer small, reviewable changes that can be cleanly abandoned or replayed.

---

## Literate Docs: Use Verso Selectively

This repo may use `verso`/`recto` for literate documentation, but only where the prose materially helps explain design decisions.

### Use Verso For

- Architecture-heavy modules
- `asupersync` and runtime selection decisions
- Node lifecycle, scheduling, backpressure, and cancellation choices
- Error-handling policy and invariants
- Any code where the "why" is likely to be forgotten later

### Do Not Use Verso For

- Trivial helpers and one-line functions
- Straightforward data transforms
- Test boilerplate
- Thin glue that is already obvious from the code

### How To Use It

- Keep the Rust source file valid and directly editable.
- Annotate only the spans that need narrative context.
- Use stable, unique fragment names for important regions.
- Keep prose focused on decisions, tradeoffs, and invariants rather than restating the code.
- If a decision changes, update the prose and the code in the same bead.

The source file remains the authority. The prose should explain why the code exists, not become a second source of truth.
...
After any substantive code changes, verify the codebase is still healthy:
...
Every meaningful module should have inline `#[cfg(test)]` unit tests alongside the implementation. Tests should cover:
- Happy path
- Edge cases: empty input, max values, boundary conditions
- Error conditions
- Any invariant that is easy to break during refactors

For Rust code with non-trivial invariants, add QuickCheck-style property tests or an equivalent property-based test suite.
...
# Run property-based tests when the change touches invariants or serialization
...
| `tests/proptest_*.rs` | Property-based tests: ID generation, hash determinism, time parsing, validation rules, other invariants |
...
A bead is a commit-sized unit of work. It is not complete until all of the following are true:

- The JJ change is named with the bead ID.
- The implementation is finished and reviewable in one change.
- Unit tests pass.
- Any relevant QuickCheck or property-based tests pass.
- `cargo clippy --all-targets -- -W clippy::pedantic -W clippy::nursery` passes.
- Dylint passes for the touched code.
- If behavior changed, docs or examples were updated in the same bead.
...
**JJ note:** Use one JJ change per bead when practical, and keep the change description tagged with the bead ID so work, rollback, and review all line up.
...
- **Change naming:** Include the bead ID in the JJ change summary or description
...
| JJ change summary | Include `br-###` for traceability |
